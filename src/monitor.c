...
/*   Updated: 2024/08/12 15:16:43 by jadyar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*monitor_routine(void *arg)
{
	...
	if (data->philos == NULL)
	{
		fprintf(stderr, "Error: data->philos is NULL\n");
		return (NULL);
	}
	while (1)
	{
		...
		{
			pthread_mutex_lock(&data->death_lock); // Locking death_lock
			if (get_time() - data->philos[i].last_eat > data->time_to_die)
			{
				print_status(&data->philos[i], "died");
				data->she_died = 1;
				// Setting she_died flag
				pthread_mutex_unlock(&data->death_lock); // Unlocking death_lock
			}
			pthread_mutex_unlock(&data->death_lock); // Unlocking death_lock
			i++;
		}
...
void	*philo_routine(void *arg)
{
	...
	{
		pthread_mutex_lock(&data->death_lock); // Locking death_lock
		if (data->she_died)                    // Checking she_died flag
		{
			pthread_mutex_unlock(&data->death_lock); // Unlocking death_lock
			break ;
		}
		...
		pthread_mutex_lock(&data->forks[(philo->id + 1) % data->philo_count]);
		print_status(philo, TAKE_FORKS);
		...
		pthread_mutex_unlock(&data->forks[(philo->id + 1) % data->philo_count]);
